...
- **Cron** — `apps/web/vercel.json` (авто-подтверждение раз в сутки); роут ждёт `Bearer $CRON_SECRET`
  (Vercel Cron шлёт его сам). Батч ограничен `take:200`, `maxDuration=300`.
...
- **Вход реальных пользователей.** Реальная доставка кода (SMS/Telegram Gateway) **не реализована** —
  это осознанно отложено (Фаза 1.5). Варианты:
  - **Демо/пилот:** `OTP_DEV_MODE=1` на Preview — входят только тест-номера `+9967000XXXXX` (в проде
    этот режим гейтом по `VERCEL_ENV` не отдаёт код — безопасно).
  - **Боевой запуск:** интегрировать провайдера (Telegram Gateway `sendVerificationMessage` ~$0.01/код
    с фолбэком на SMS-агрегатора КР) синхронно в `requestOtp`. Требует твой аккаунт провайдера.
    Без него на публичном Production живые пользователи войти не смогут.
